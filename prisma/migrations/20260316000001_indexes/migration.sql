...
CREATE INDEX "WaitlistQuestion_campaignId_idx" ON "WaitlistQuestion"("campaignId");

-- WaitlistResponse: review queue — filter pending/approved responses per campaign
CREATE INDEX "WaitlistResponse_campaignId_status_idx" ON "WaitlistResponse"("campaignId", "status");

-- WaitlistBan: FK lookup — check if a user is banned from a campaign's waitlist
CREATE INDEX "WaitlistBan_campaignId_idx" ON "WaitlistBan"("campaignId");
